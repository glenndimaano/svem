...
const updateDashboardURLs = async (apiKey, appUrl) => {
    ...
    const redirectURLs = appUrl.endsWith("/")
        ? [`${appUrl}api/auth`, `${appUrl}api/auth/callback`, `${appUrl}shopify/auth`, `${appUrl}shopify/callback`]
        : [`${appUrl}/api/auth`, `${appUrl}/api/auth/callback`, `${appUrl}/shopify/auth`, `${appUrl}/shopify/auth/callback`];
